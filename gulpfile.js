...
gulp.task('copy:fonts', function () {
  ...
  return gulp.src(dirs.srcPath + '/fonts/**/*')
...
gulp.task('copy:webfonts', function () {
  console.log('---------- Копирование шрифтов');
  return gulp.src(dirs.srcPath + '/webfonts/*.{ttf,woff,woff2,eot,svg}')
    // .pipe(newer(dirs.buildPath + '/webfonts'))  // оставить в потоке только изменившиеся файлы
    .pipe(size({
      title: 'Размер',
      showFiles: true,
      showTotal: false,
    }))
    .pipe(gulp.dest(dirs.buildPath + '/webfonts'));
});
...
gulp.task('build', function (callback) {
  gulpSequence(
    ['clean'],
    ['style', 'style:single', 'js', 'copy:css', 'copy:img', 'copy:js', 'copy:fonts', 'copy:webfonts'],
    'html',
    callback
  );
});
...
gulp.task('serve', ['build'], function() {
  ...
  gulp.watch('/webfonts/*.{ttf,woff,woff2,eot,svg}', {cwd: dirs.srcPath}, ['watch:webfonts']);
  // Слежение за html
  gulp.watch([
    '*.html',
    '_include/*.html',
...
gulp.task('watch:webfonts', ['copy:webfonts'], reload);
